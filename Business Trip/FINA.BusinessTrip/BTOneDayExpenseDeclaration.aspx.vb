...
Partial Public Class BTOneDayExpenseDeclaration
    ...
    Protected _isOneDayExpense As Boolean
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            ...
            If _dtBTData IsNot Nothing AndAlso _dtBTData.Rows.Count > 0 Then
                ...
                _invEnable = _isFI AndAlso finStatus <> FIStatus.completed.ToString()
                _airEnable = _isGA AndAlso isBudgetChecked
                _isOneDayExpense = CommonFunction._ToInt(drData("IsOneDayExpense")) > 0
                ...
                hIsOneDayExpense.Value = If(_isOneDayExpense, "T", "F")
                '
                spanExportBT.Visible = isOwner OrElse _isAdministrator OrElse isEmp OrElse isTimeKeeper OrElse _isGA
                spanExportPayment.Visible = isOwner OrElse _isAdministrator OrElse isEmp OrElse isTimeKeeper OrElse _isGA
            Else
                _enable = True
                'spanExportBT.Visible = False
                'spanExportPayment.Visible = False
                _invEnable = _isFI
                _airEnable = False
            End If
    ...
    Private Sub LoadExpenseTotalSummary(Optional ByVal reset As Boolean = False)
        If reset Then
            lblExpenseDailyAllowance.Text = "0"
            lblExpenseMovingTimeAllowance.Text = "0"
            lblExpenseOtherExpense.Text = "0"
            lblExpenseTotalExpense.Text = "0"
            lblCashAdvance.Text = "0"
            lblCreditAdvance.Text = "0"
            lblDisparity.Text = "0"
    ...
    Private Sub GetDataTable()
        ...
        Else
            ...
            _dtData = ExpenseProvider.tbl_BT_Expense_Search(btType, fullName, empCode, budgetCode, _username, btNo, departureFrom, departureTo, "oneday")
        End If
    End Sub
    ...
    Private Sub LoadBTRegisterInfo()
        ...
        If dtData.Rows.Count > 0 Then
            ...
            End If
            '
            trCommonCredit.Visible = chkCredit.Checked
            trOtherCredit.Visible = chkCredit.Checked
        ...
        If role.ToLower() = RoleType.Finance.ToString().ToLower() _
            ...
            If _dtData.Rows.Count > 0 Then
                ...
                If isSubmit Then
                    ...
                    End If
                    '
                    If _isOneDayExpense AndAlso (status.ToLower() = FIStatus.pending.ToString() OrElse status.ToLower() = FIStatus.budget_reconfirmed.ToString()) Then
                        ...
                        End If
                    ElseIf budgetChecked AndAlso ((Not _isOneDayExpense AndAlso status.ToLower() = FIStatus.pending.ToString()) OrElse status.ToLower() = FIStatus.checked.ToString() OrElse status.ToLower() = FIStatus.completed.ToString()) AndAlso oraStatus.ToLower() <> "done" AndAlso oraStatus.ToLower() <> "paid" Then
    ...
    Private Sub EnableForm(ByVal enable As Boolean)
        ...
        'chkCredit.Enabled = enable AndAlso _isOneDayExpense
    ...
    Private Sub LoadBTExpenseRequest(Optional ByVal clear As Boolean = False)
        Dim dtTransData As DataTable = If(clear, Nothing, ExpenseProvider.BTExpenseRequest_Search(CommonFunction._ToInt(hID.Value)))
        CommonFunction.LoadDataToGrid(grvBTRequest, dtTransData, "", Nothing, "EnableForm", _enable)
        '
        LoadExpenseTotalSummary()
    End Sub

    Private Sub LoadBTInvoice(Optional ByVal clear As Boolean = False)
        Dim dtData As DataTable = If(clear, Nothing, InvoiceProvider.BTInvoice_Search(CommonFunction._ToInt(hID.Value)))
        If Not clear Then
    ...
    Private Sub LoadBTExpenseRequestByID()
        ...
        Dim dtData As DataTable = ExpenseProvider.BTExpenseRequest_GetByID(requestID)
        If dtData.Rows.Count > 0 Then
            Dim drData As DataRow = dtData.Rows(0)
            Dim ddate As DateTime = CommonFunction._ToDateTime(drData("oFromDate"))
            ...
            txeFromTime.DateTime = CommonFunction._ToDateTime(drData("oFromDate"))
            txeToTime.DateTime = CommonFunction._ToDateTime(drData("oToDate"))
            'request details     
            txtTaxiQty.Value = CommonFunction._ToUnsignIntWithNull(drData("oTaxiTime"))
            txtMotobikeQty.Value = CommonFunction._ToUnsignIntWithNull(drData("oMotobikeTime"))
            txtTaxiFee.Value = CommonFunction._ToMoneyWithNull(drData("oTaxiAmount"))
            txtMotobikeFee.Value = CommonFunction._ToMoneyWithNull(drData("oMotobikeAmount"))
            ...
            '
            Dim breakfastUnit As Decimal = CommonFunction._ToMoney(drData("BreakfastAmount"))
            txtBreakfastUnit.Value = If(breakfastUnit > 0, breakfastUnit, CommonFunction._ToMoney(hBreakfastUnit.Value))
            Dim lunchUnit As Decimal = CommonFunction._ToMoney(drData("LunchAmount"))
            txtLunchUnit.Value = If(lunchUnit > 0, lunchUnit, CommonFunction._ToMoney(hLunchUnit.Value))
            Dim dinnerUnit As Decimal = CommonFunction._ToMoney(drData("DinnerAmount"))
            txtDinnerUnit.Value = If(dinnerUnit > 0, dinnerUnit, CommonFunction._ToMoney(hDinnerUnit.Value))
            ...
            chkCarRequest.Checked = CommonFunction._ToBoolean(drData("oCarRequest"))
            chkAirTicketRequest.Checked = CommonFunction._ToBoolean(drData("oAirTicketRequest"))
            chkTrainTicketRequest.Checked = CommonFunction._ToBoolean(drData("oTrainTicketRequest"))
            txtTotalTransport.Value = CommonFunction._ToMoney(drData("TotalTransportation"))
            txtTaxiDesc.Text = CommonFunction._ToString(drData("oTaxiDesc"))
            txtMotobikeDesc.Text = CommonFunction._ToString(drData("oMotobikeDesc"))
            '
            ...
            Else
                'spanMovingTime.Visible = True
                chkMovingTimeAllowance.Checked = CommonFunction._ToMoney(drData("oMovingTimeAllowance")) > 0
            End If
            'chkMovingTimeChange()
            Dim movingTimeAmount As Decimal = CommonFunction._ToMoney(drData("oMovingTimeAllowance"))
            ...
            End If
            '
            Dim requestDate As DateTime = CommonFunction._ToDateTime(drData("Date"))
            ...
            Select Case CommonFunction._ToString(item("AttachmentType"))
                Case "register"
                    ...
                    If _isOneDayExpense Then
                        ...
                        tdPaymentOrder.InnerText = "2"
                        tdOtherOrder.InnerText = "3"
                        panExpenseAttachments.Controls.Add(ltrRegister)
                        expendDesc = CommonFunction._ToString(item("Description"))
                    Else
                        trApproval.Visible = False
                        tdPaymentOrder.InnerText = "1"
                        tdOtherOrder.InnerText = "2"
                        'panRegisterAttachments.Controls.Add(ltrRegister)
                        desc = CommonFunction._ToString(item("Description"))
                    End If
                Case "expense"
                    'If withDesc Then txtAttRegisterDesc.Text = desc
                    Dim ltrPayment As New Literal()
                    ltrPayment.Text = String.Format("<ol class='attachments'>{0}</ol>", li)
                    panPaymentAttachments.Controls.Add(ltrPayment)
                    expendDesc = CommonFunction._ToString(item("Description"))
                Case "other"
                    desc = CommonFunction._ToString(item("Description"))
    ...
    Private Sub EnableInfoForm(ByVal enable As Boolean)
        ...
        '
        ddlBudgetName.Enabled = _isOneDayExpense AndAlso (_enable OrElse _enableBudget)
        ddlBudgetName.AutoPostBack = _isOneDayExpense AndAlso (_enable OrElse _enableBudget)
        If Not (_isOneDayExpense AndAlso (_enable OrElse _enableBudget)) Then
            ddlBudgetName.Attributes("onchange") = "BudgetCodeChange(this, 'txtBudgetCode');"
        End If
        chkBudgetAll.Enabled = _isOneDayExpense AndAlso (_enable OrElse _enableBudget)
        txtProjectBudgetCode.ReadOnly = Not _isOneDayExpense OrElse (Not _enable AndAlso Not _enableBudget)
        '
        InitSearchBudgetName(Not (_isOneDayExpense AndAlso (_enable OrElse _enableBudget)), chkBudgetAll.Checked, Not (_isOneDayExpense AndAlso (_enable OrElse _enableBudget)))
    End Sub
    ...
    Private Sub ClearRequestForm()
        ...
        'Dim dtDefault As DateTime = DateTime.Now
    ...
    Private Sub ClearAttachmentForm(Optional ByVal withDesc As Boolean = True)
        ...
        panPaymentAttachments.Controls.Clear()
        If withDesc Then
    ...
    Private Function GetExpenseObject() As tblBTExpenseInfo
    ...
    Private Function GetRequestObject() As tblBTExpenseRequestInfo
        ...
        obj.CreatedBy = _username
        obj.ModifiedBy = _username
        obj.oFromDate = New DateTime(dteDate.Date.Year, dteDate.Date.Month, dteDate.Date.Day, txeFromTime.DateTime.Hour, txeFromTime.DateTime.Minute, 0)
        obj.oToDate = New DateTime(dteDate.Date.Year, dteDate.Date.Month, dteDate.Date.Day, txeToTime.DateTime.Hour, txeToTime.DateTime.Minute, 0)
        obj.DDate = dteRequestDate.Date
        ...
        obj.BreakfastAmount = CommonFunction._ToMoney(txtBreakfastAmount.Text.Trim())
        obj.LunchAmount = CommonFunction._ToMoney(txtLunchAmount.Text.Trim())
        obj.DinnerAmount = CommonFunction._ToMoney(txtDinnerAmount.Text.Trim())
        obj.oTaxiTime = CommonFunction._ToUnsignInt(txtTaxiQty.Text.Trim())
        obj.oTaxiAmount = CommonFunction._ToMoney(txtTaxiFee.Text.Trim())
        If obj.oTaxiAmount > 0 AndAlso obj.oTaxiTime <= 0 Then
            obj.oTaxiTime = 1
        End If
        obj.oMotobikeTime = CommonFunction._ToUnsignInt(txtMotobikeQty.Text.Trim())
        obj.oMotobikeAmount = CommonFunction._ToMoney(txtMotobikeFee.Text.Trim())
        If obj.oMotobikeAmount > 0 AndAlso obj.oMotobikeTime <= 0 Then
            obj.oMotobikeTime = 1
        End If
        obj.oCarRequest = chkCarRequest.Checked
        obj.oAirTicketRequest = chkAirTicketRequest.Checked
        obj.oTrainTicketRequest = chkTrainTicketRequest.Checked
        obj.oTaxiDesc = txtTaxiDesc.Text.Trim()
        obj.oMotobikeDesc = txtMotobikeDesc.Text.Trim()
        obj.AllowanceCurrency = "vnd"
        obj.AllowanceExrate = 1
        obj.HotelAmount = 0
        obj.HotelCurrency = "vnd"
        obj.HotelExrate = 1
        obj.CreditAmount = If(chkCommonCCAmount.Checked, CommonFunction._ToMoney(spiCommonCCAmount.Text.Trim()), 0)
        Dim totalCommon As Decimal = obj.BreakfastAmount + obj.DinnerAmount + obj.LunchAmount + obj.oTaxiAmount + obj.oMotobikeAmount
        If obj.CreditAmount > totalCommon Then
            obj.CreditAmount = totalCommon
        End If
        Return obj
    End Function
    ...
    Protected Sub btnSearch_OnClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnSearch.Click
        ...
        Try
            ...
            postBackUrl.Append("~/BTOneDayExpenseDeclaration.aspx")
    ...
    Protected Sub btnFinish_OnClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnFinish.Click
        ...
        Try
            Dim objExpense As tblBTExpenseInfo = GetExpenseObject()
            UploadAttachments()
            'hID.Value = ""            
            ExpenseProvider.BTExpense_Update(objExpense)
            '
            LoadBTRegisterInfo()
            LoadBTExpenseInfo()
            '            
    ...
    Protected Sub btnSubmit_OnClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnSubmit.Click
        ...
        Try
            ...
            If chkBudgetAll.Checked AndAlso hIsOneDayExpense.Value = "T" Then
    ...
    Protected Sub btnAdd_OnClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnAdd.Click
        ...
        Try
            ...
            spanExportPayment.Visible = True
            'pre init
            ...
            'hIsDGMAndAbove.Value = If(Array.IndexOf(_dgmAndAbove, txtPosition.Text.Trim().ToLower()) >= 0, "Y", "N")
            hIsGMAndAbove.Value = If(Array.IndexOf(_gmAndAbove, txtPosition.Text.Trim().ToLower()) >= 0, "Y", "N")
            hIsOneDayExpense.Value = "T"
            '
            Dim obj As tblBTRegisterInfo = GetObject()
            Dim dtResult As DataTable = BusinessTripProvider.BTRegister_Insert(obj, 1)
    ...
    Protected Sub btnDeleteMany_OnClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
        ...
        Try
            ...
            If deleteUsers.Trim().Length > 0 Then
                ExpenseProvider.BTExpense_Delete(deleteUsers)
                LoadDataList()
            End If
    ...
    Protected Sub btnSaveRequest_OnClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnSaveRequest.Click
        ...
        Try
            ...
            If requestObj.ID > 0 Then
                ExpenseProvider.BTExpenseRequest_Update(requestObj, chkMovingTimeAllowance.Checked)
                LoadBTExpenseRequestByID()
            Else
                ExpenseProvider.BTExpenseRequest_Insert(requestObj, chkMovingTimeAllowance.Checked)
            End If
            '
    ...
    Private Function IsValidCommon(ByVal obj As tblBTExpenseRequestInfo) As Boolean
        Dim isValid As Boolean = True
        If obj.oToDate <= obj.oFromDate Then
            ...
            isValid = False
        ElseIf ExpenseProvider.BTExpenseRequest_IsOneDayBTTimeConflict(obj.BTExpenseID, obj.ID, obj.oFromDate, obj.oToDate) Then
            CommonFunction.ShowErrorMessage(panMessage, "BT time conflict!")
            dteDate.Attributes("style") = "border-color: red;"
            txeFromTime.Attributes("style") = "border-color: red; width: 100px !important; display: inline-block"
            txeToTime.Attributes("style") = "border-color: red; width: 100px !important; display: inline-block"
            isValid = False
        ElseIf chkMovingTimeAllowance.Checked AndAlso Not ExpenseProvider.BTExpenseRequest_CheckMovingTimeOneDay(obj.ID, obj.BTExpenseID, obj.oFromDate) Then
            CommonFunction.ShowErrorMessage(panMessage, String.Format("Moving time allowance for {0} is declared!", obj.oFromDate.ToString("dd-MMM-yyyy")))
            ...
            '        If obj.BreakfastAmount > BreakfastNorm Then
            '            isValid = False
            '        End If
            '        If obj.LunchAmount > LunchNorm Then
            '            isValid = False
            '        End If
            '        If obj.DinnerAmount > DinnerNorm Then
            '            isValid = False
            '        End If
            '        If Not isValid Then
            '            CommonFunction.ShowErrorMessage(panMessage, "Meal fee is over policy!")
            '        Else
            '            Dim motobikeQty As Integer = CommonFunction._ToUnsignInt(txtMotobikeQty.Text.Trim())
            '            If motobikeQty < 1 Then
            '                motobikeQty = 1
            '            End If
            '            If obj.oMotobikeAmount / motobikeQty > MotobikeNorm Then
    ...
    Protected Sub btnDelete_OnClick(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Try
            ...
            ExpenseProvider.BTExpense_Delete(btID.ToString())
            CommonFunction.ShowInfoMessage(panMessage, "Your data is deleted successfully!")
            '
            LoadDataList()
        Catch ex As Exception
    ...
    Protected Sub btnCancel_OnClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnCancel.Click
        ...
        Try
            ...
            'EnableInfoForm(True)
            'ShowInfoRows(False)
            'LoadBTExpenseRequest(True)
            'ClearRequestForm()
            'LoadBTExpenseOther(True)
            'ClearOtherForm()
            'ClearAttachmentForm()
            'LoadBTInvoice(True)
            'ClearInvoiceForm()
    ...
    Private Sub LoadExpenseNorm()
        ...
        If dtNorm.Rows.Count > 0 Then
            ...
            'Dim motobikeQty As Integer = CommonFunction._ToInt(txtMotobikeQty)
            'If motobikeQty < 1 Then
            '    motobikeQty = 1
            'End If
    ...
    Protected Sub btnExportBT_OnClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnExportBT.Click
        ...
        Try
            ...
            'business
            Dim ds As DataSet = ReportProvider.GetOneDayExpenseBT(CommonFunction._ToInt(hID.Value))
    ...
    Protected Sub btnExportForPayment_OnClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnExportPayment.Click
        CommonFunction.SetPostBackStatus(btnExportPayment)
        Dim xlApp As Excel.Application = Nothing
        Dim xlWorkBook As Excel.Workbook = Nothing
        Dim xlWorkSheet As Excel.Worksheet = Nothing
        Try
            Dim tmp As String = Server.MapPath("\Export\Template\bt_expense_template.xlt")            '            
            Dim misValue As Object = System.Reflection.Missing.Value
            '
            xlApp = New Excel.ApplicationClass()
            xlWorkBook = xlApp.Workbooks.Open(tmp, 0, True, 5, "", "", True, Microsoft.Office.Interop.Excel.XlPlatform.xlWindows, "\t", False, False, 0, True, 1, 0)
            xlWorkSheet = xlWorkBook.Worksheets(1)
            'business
            Dim ds As DataSet = ReportProvider.GetOneDayPayment(CommonFunction._ToInt(hID.Value))
            ds.Tables(0).TableName = "General"
            ds.Tables(1).TableName = "Details"
            ds.Tables(2).TableName = "DetailsSummary"
            ds.Tables(3).TableName = "Other"
            ds.Tables(4).TableName = "OtherSummary"
            ds.Tables(5).TableName = "Exrate"
            '
            Dim rowIndex As Integer = 3
            'general info            
            Dim drGeneral As DataRow = ds.Tables("General").Rows(0)
            ...
            For i As Integer = 0 To dtDetails.Rows.Count - 1
                ...
                xlWorkSheet.Cells(rowIndex, colIndex) = drDetails("Dinner")
                colIndex += 1
                xlWorkSheet.Cells(rowIndex, colIndex) = drDetails("OtherMeal")
                colIndex += 1

                xlWorkSheet.Cells(rowIndex, colIndex) = drDetails("TotalAmount")
                colIndex += 1
                xlWorkSheet.Cells(rowIndex, colIndex) = drDetails("MovingTimeAllowance")
                colIndex += 1
            ...
            rowIndex = nextRowIndex
            xlWorkSheet.Cells(rowIndex, 6) = drDetailsSummary("TotalMealAmount")
            ...
            For i As Integer = 0 To dtOther.Rows.Count - 1
                ...
                xlWorkSheet.Cells(rowIndex, colIndex) = drOther("RequestDate")
                colIndex += 1
                xlWorkSheet.Cells(rowIndex, colIndex) = drOther("ChiPhi")
                colIndex += 6
                xlWorkSheet.Cells(rowIndex, colIndex) = drOther("Tiente")
                colIndex += 1
                xlWorkSheet.Cells(rowIndex, colIndex) = drOther("Sotien")
                colIndex += 1
            ...
            xlWorkSheet.Cells(rowIndex, 9) = drOtherSummary("TotalChiPhiKhac")

            'summary            
            rowIndex = rowIndex + 2
            Dim drSummary As DataRow = ds.Tables("OtherSummary").Rows(0)
            xlWorkSheet.Cells(rowIndex, 6) = drSummary("TongTien")
            rowIndex = rowIndex + 1
            xlWorkSheet.Cells(rowIndex, 6) = drSummary("TamUng")
            rowIndex = rowIndex + 1
            ...
            '
            xlWorkBook.SaveAs(Server.MapPath(filePath), Excel.XlFileFormat.xlWorkbookNormal, misValue, misValue, False, False, Excel.XlSaveAsAccessMode.xlNoChange, Excel.XlSaveConflictResolution.xlUserResolution, True, misValue, misValue, misValue)
            btnExportPayment.Attributes("data-file-path") = filePath.Substring(1)
            '            
            xlWorkBook.Close(False)
            xlApp.Quit()
            CommonFunction.ReleaseObject(xlWorkSheet)
            ...
            Select Case role.ToLower()
                Case RoleType.Finance.ToString().ToLower(), RoleType.Administrator.ToString().ToLower(), RoleType.Finance_GA.ToString().ToLower()
                    ...
                    Else
                        ...
                        Dim dr As DataRow = ExpenseProvider.tbl_BT_Expense_UpdateStatusFI(BTRegisterID, FIStatus.completed.ToString(), _username, "", supplierNo, supplierSite, glDate, batchName, invoiceDate)
    ...
    '        Dim dtBT As DataTable = ExpenseProvider.BTExpenseRequest_GetByID(CommonFunction._ToInt(hRequestID.Value))
    '        If dtBT.Rows.Count > 0 Then
    '            Dim drBT As DataRow = dtBT.Rows(0)
    '            movingTimeAmount = CommonFunction._ToMoney(drBT("oMovingTimeAllowance"))
    '            movingTimeCurrency = CommonFunction._ToString(drBT("oMovingTimeAllowanceCurrency"))
    '        End If
    '        If movingTimeAmount <= 0 Then
    '            Dim dtPolicy As DataTable = mMovingTimeAllowanceProvider.m_Expense_GetPolicy("VN")
    '            If dtPolicy.Rows.Count > 0 Then
    ...
    Protected Sub btnConfirmBudget_OnClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnConfirmBudget.Click
        ...
        Try
            ...
            Dim message As String = ExpenseProvider.BTExpense_ConfirmBudget(obj, comment)
            If message.Length = 0 Then
                LoadBTRegisterInfo()
                LoadBTExpenseInfo()
                'Send notice email
    ...
    Protected Sub btnBudgetReject_OnClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnBudgetReject.Click
        ...
        Try
            ...
            End If
            Dim obj As tblBTRegisterInfo = GetObject()
            Dim message As String = ExpenseProvider.BTExpense_ConfirmRequester(obj, rejectReason)
            If message.Length = 0 Then
    ...
    Protected Sub btnRejectToBudget_OnClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnRejectToBudget.Click
        ...
        Try
            ...
            Dim message As String = ExpenseProvider.BTExpense_RejectToBudget(obj.BTRegisterID, comment, obj.ModifiedBy)
            If message.Length > 0 Then
                CommonFunction.ShowErrorMessage(panMessage, message)
            Else
                LoadBTRegisterInfo()
    ...
    Protected Sub btnCopy_OnClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnCopy.Click
        ...
        Try
            ...
            Dim count As Integer = ExpenseProvider.BTExpense_Copy(btID, employees, _username)
            LoadDataList()
    ...
    Private Sub LoadBTExpenseOther(Optional ByVal clear As Boolean = False)
        Dim dtData As DataTable = If(clear, Nothing, ExpenseProvider.BTExpenseOther_Search(CommonFunction._ToInt(hID.Value)))
    ...
    Private Sub ClearOtherForm()
        ...
        'dteExpenseOtherDate.Date = DateTime.Now
        ...
        '        
        'If _isDomestic Then
        '    CommonFunction.SetCBOValue(ddlExpenseOtherCurrency, "vnd")
        'Else
        '    CommonFunction.SetCBOValue(ddlExpenseOtherCurrency, "usd")
        'End If
        'spiExpenseOtherExrate.Value = ExpenseProvider.GetExrate("USD", ddlExpenseOtherCurrency.SelectedValue, dteExpenseOtherDate.Date)
    End Sub
    ...
    Private Function GetOtherObject() As tblBTExpenseOtherInfo
        ...
        obj.Currency = "vnd" 'ddlExpenseOtherCurrency.SelectedValue
        obj.Exrate = 1 'CommonFunction._ToMoney(spiExpenseOtherExrate.Text.Trim())
        'If obj.Exrate = 0 Then
        '    obj.Exrate = 1
        'End If
...
#Region "Send Notice Email"
    ...
    Private Function SendSubmitEmail() As Boolean
        ...
        If _sendEmailMode = SendEmailMode.UserTest.ToString().ToLower() Then
            eTo = UserProvider.UserTestEmail_Get(CommonFunction._ToInt(hID.Value), If(_isOneDayExpense, RoleType.Finance_Budget.ToString(), RoleType.Finance.ToString()))
        Else
            ...
            End If
        End If
        Dim backUrl As String = If(_isOneDayExpense, "FiBudgetChecking.aspx", "FiExpenseMgmt.aspx")
        Dim viewLink As String = String.Format("BTOneDayExpenseDeclaration.aspx?id={0}&back={1}&params=btid;eq;{0};amp;bttype;eq;;amp;loc;eq;;amp;fdate;eq;;amp;tdate;eq;;amp;div;eq;;amp;dep;eq;;amp;sec;eq;;amp;group;eq;;amp;ecode;eq;;amp;ename;eq;;amp;btno;eq;", hID.Value, backUrl)
    ...
    Private Function SendConfirmBudgetEmail() As Boolean
        ...
        Else
            ...
            End If
        End If
        Dim viewLink As String = String.Format("BTOneDayExpenseDeclaration.aspx?id={0}&back=FiExpenseMgmt.aspx&params=btid;eq;{0};amp;bttype;eq;;amp;loc;eq;;amp;fdate;eq;;amp;tdate;eq;;amp;div;eq;;amp;dep;eq;;amp;sec;eq;;amp;group;eq;;amp;ecode;eq;;amp;ename;eq;;amp;btno;eq;", hID.Value)
    ...
    Private Function SendUserEmail() As Boolean
        ...
        Else
            ...
            End If
        End If
        Dim viewLink As String = String.Format("BTOneDayExpenseDeclaration.aspx?id={0}&back=BTOneDayExpenseDeclaration.aspx", hID.Value)
        Dim eBody As String = GenNoticeBody(viewLink)
        Return SendNoticeEmail(eTo, eBody)
    End Function
