...
#Region "InitForm"
    ...
    Private Sub SetPreParams()
        ...
        If btid > 0 Then
            ...
            Dim noAdvancePageSize As Integer = CommonFunction._ToUnsignInt(Request.QueryString("npsize"))
            If noAdvancePageSize < 1 Then
                noAdvancePageSize = 100
            End If
            grvBTNoAdvance.SettingsPager.PageSize = noAdvancePageSize
            ...
            Dim noAdvancePage As Integer = CommonFunction._ToUnsignInt(Request.QueryString("npage"))
            grvBTNoAdvance.PageIndex = noAdvancePage
...
#End Region
    ...
    Private Sub GetDataTable()
        ...
        '
        _dtData = BusinessTripProvider.tbl_BT_Register_SearchFI(BTTypeID, LocationID, DivID, DepID, SecID, GropID, FullName, EmployeeCode, FromDate, ToDate, "", -1, "", _username, True, btNo, btsStatus)
    End Sub
    ...
    Private Sub LoadRegister()
        CommonFunction.LoadDataToGrid(grvBTRegister, _dtData, "IsSubmited = 1 and (IsOneDayExpense is null or IsOneDayExpense <> 1) and FIStatus <> '" + FIStatus.completed.ToString() + "' and FIStatus <> '" + FIStatus.rejected.ToString() + "' and FIStatus <> '" + FIStatus.cancelled.ToString() + "'", "No")
    End Sub
    ...
    Private Sub LoadNoAdvance()
        CommonFunction.LoadDataToGrid(grvBTNoAdvance, _dtData, "IsSubmited = 1 and (IsOneDayExpense is null or IsOneDayExpense <> 1) and BTAdvance <= 0 and FIStatus = '" + FIStatus.completed.ToString() + "'", "No")
    End Sub

    Private Sub LoadSubmitted()
        CommonFunction.LoadDataToGrid(grvBTSubmitted, _dtData, "IsSubmited = 1 and (IsOneDayExpense is null or IsOneDayExpense <> 1) and BTAdvance > 0 and FIStatus = '" + FIStatus.completed.ToString() + "'", "No")
    End Sub
    ...
    Protected Sub btnApprove_OnClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnApprove.Click
        ...
        Try
            ...
            Else
                ...
                Dim dr As DataRow = BusinessTripProvider.tbl_BT_Register_UpdateStatusFI(_btID, FIStatus.completed.ToString(), _username, "", supplierNo, supplierSite, glDate, batchName, invoiceDate)
    ...
    Protected Sub btnRejectOK_OnClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnRejectOK.Click
        ...
        Try
            ...
            If rejectReason.Trim().Length = 0 Then
                Return
            End If
            'Dim oracleError As Boolean = BusinessTripProvider.tbl_BT_Register_CheckOracleStatus(_btID)
            'If oracleError Then

            'Else
            '    CommonFunction.ShowErrorMessage(panMessage, "Can not reject this BT! Please check oracle invoice status!")
            'End If
            Dim dr As DataRow = BusinessTripProvider.tbl_BT_Register_UpdateStatusFI(_btID, FIStatus.rejected.ToString(), _username, rejectReason)
            Dim message As String = CommonFunction._ToString(dr("MessageResult")).Trim()
            If message.Trim().Length > 0 Then
    ...
    Protected Sub grvBTNoAdvance_OnBeforeGetCallbackResult(ByVal sender As Object, ByVal e As EventArgs) Handles grvBTNoAdvance.BeforeGetCallbackResult
        GetDataTable()
        LoadNoAdvance()
    End Sub
        ...
        Try
            ...
            If dtRegister.Rows.Count > 0 Then
                ...
                Dim params As String = String.Format( _
                    "btid={0}&bttype={1}&loc={2}&fdate={3}&tdate={4}&div={5}&dep={6}&sec={7}&group={8}&ecode={9}&ename={10}&rpage={11}&rpsize={12}&spage={13}&spsize={14}&btno={15}&npsize={16}&npage={17}&jpsize={18}&jpage={19}", _
                    ...
                    grvBTSubmitted.PageIndex, grvBTSubmitted.SettingsPager.PageSize, hBTNo.Value, grvBTNoAdvance.SettingsPager.PageSize, grvBTNoAdvance.PageIndex, grvBTRejected.SettingsPager.PageSize, grvBTRejected.PageIndex)
                postBackUrl.Append(String.Format("?id={0}&back=FiAdvanceMgmt.aspx&params={1}", btID, params.Replace("&", ";amp;").Replace("=", ";eq;")))
...
#Region "Send Notice Email"
    ...
    Private Function GenNoticeBody(Optional ByVal link As String = "") As String
        ...
        Dim requestGA As String = ""
        Dim comment As String = ""
        Dim ExpectedDepartureTime As DateTime
        Dim ExpectedDepartureFlightNo As String = ""
        Dim ExpectedReturnTime As DateTime
        Dim ExpectedReturnFlightNo As String = ""
        Dim expectedDeparture As String = ""
        Dim expectedReturn As String = ""
        '
        Dim dtRegister As DataTable = BusinessTripProvider.BTRegister_GetByID(_btID)
        If dtRegister.Rows.Count > 0 Then
            Dim dr As DataRow = dtRegister.Rows(0)
            ...
            departureDate = CommonFunction._ToDateTime(If(isOneday, dr("FromDate"), dr("DepartureDate")))
            returnDate = CommonFunction._ToDateTime(If(isOneday, dr("ToDate"), dr("ReturnDate")))
            ...
            btAmount = String.Format("{0} {1}", CommonFunction._FormatMoney(dr("AdvanceAmount")), dr("BTCurrency"))
            requestGA = CommonFunction._ToString(dr("RequestGA"))
            comment = CommonFunction._ToString(dr("RejectReasonFI"))
            ExpectedDepartureTime = CommonFunction._ToDateTime(dr("ExpectedDepartureTime"))
            ExpectedDepartureFlightNo = CommonFunction._ToString(dr("ExpectedDepartureFlightNo"))
            ExpectedReturnTime = CommonFunction._ToDateTime(dr("ExpectedReturnTime"))
            ExpectedReturnFlightNo = CommonFunction._ToString(dr("ExpectedReturnFlightNo"))
            'expect air ticket
            ...
            End If
            '
            Dim url = If(CommonFunction._ToString(dr("BTType")).IndexOf("oneday_") = 0, "BTOneDayDeclaration.aspx", "BTAdvanceDeclaration.aspx")
        ...
        eBody.Append(String.Format("<tr><td valign='top'><ul><li style='margin: 0;'><strong>Request to GA:</strong></li></ul></td><td><span style='color: red'>{0}</span><span style='color: #0070c0'>{1}{2}</span></td></tr>", If(requestGA.Trim().Length > 0, requestGA, "Don't have any request"), If(requestGA.Trim().Length > 0 AndAlso expectedDeparture.Trim().Length > 0, String.Concat("<br />* Expected Departure - ", expectedDeparture), ""), If(requestGA.Trim().Length > 0 AndAlso expectedReturn.Trim().Length > 0, String.Concat("<br />* Expected Return - ", expectedReturn), "")))
    ...
    Private Function SendNoticeEmail(ByVal eTo As String, ByVal eBody As String) As Boolean
        ...
        If eTo.Trim().Length > 0 Then
            ...
            Dim eSubject As String = String.Format("{0}[BTS Advance]: {1} - {2}/{3} (Status: {4})", indicator, _btNo, _employeeName, _lastDestination, _fiStatus)
            Try
    ...
    Private Function SendUserEmail() As Boolean
        ...
        Else
            ...
            End If
        End If
        Dim viewLink As String = String.Concat("{0}?id=", _btID, "&back={0}", _btID)
