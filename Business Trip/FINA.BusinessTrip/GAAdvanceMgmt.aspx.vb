...
Partial Public Class GAAdvanceMgmt
...
#Region "InitForm"
    ...
    Private Sub SetPreParams()
        ...
        If btid > 0 Then
            ...
            Dim noRequestPageSize As Integer = CommonFunction._ToUnsignInt(Request.QueryString("qpsize"))
            If noRequestPageSize < 1 Then
                noRequestPageSize = 100
            End If
            grvBTNoRequest.SettingsPager.PageSize = noRequestPageSize
            ...
            Dim noRequestPage As Integer = CommonFunction._ToUnsignInt(Request.QueryString("qpage"))
            grvBTNoRequest.PageIndex = noRequestPage
...
#Region "Event"
    ...
    Private Sub GetDataTable()
        ...
        _dtData = BusinessTripProvider.tbl_BT_Register_SearchGA(BTTypeID, LocationID, DivID, DepID, SecID, GropID, FullName, EmployeeCode, FromDate, ToDate, "", -1, "", strUser, True)
    End Sub
    ...
    Private Sub LoadRegister()
        CommonFunction.LoadDataToGrid(grvBTRegister, _dtData, "IsSubmited = 1 and RequestGA <> '' and FIStatus <> '" + FIStatus.completed.ToString() + "' and FIStatus <> '" + FIStatus.rejected.ToString() + "'", "No") 'and RequestGA <> ''
    End Sub

    Private Sub LoadNoRequest()
        CommonFunction.LoadDataToGrid(grvBTNoRequest, _dtData, "IsSubmited = 1 and RequestGA = '' and FIStatus <> '" + FIStatus.completed.ToString() + "' and FIStatus <> '" + FIStatus.rejected.ToString() + "'", "No")
    End Sub

    Private Sub LoadRejected()
        CommonFunction.LoadDataToGrid(grvBTRejected, _dtData, "IsSubmited = 1 and FIStatus = '" + FIStatus.rejected.ToString() + "'", "No") 'and RequestGA <> ''
    End Sub

    Private Sub LoadSubmitted()
        CommonFunction.LoadDataToGrid(grvBTSubmitted, _dtData, "IsSubmited = 1 and FIStatus = '" + FIStatus.completed.ToString() + "'", "No") 'and RequestGA <> ''
    End Sub
    ...
    '        BusinessTripProvider.tbl_BT_Register_UpdateStatusGA(BTRegisterID, GAStatus.completed.ToString(), strUser, "")
    '        LoadDataGrid()
    '        '
    ...
    Protected Sub grvBTNoRequest_OnBeforeGetCallbackResult(ByVal sender As Object, ByVal e As EventArgs) Handles grvBTNoRequest.BeforeGetCallbackResult
        GetDataTable()
        LoadNoRequest()
    End Sub
    ...
    '            Return
    '        End If
    '        BusinessTripProvider.tbl_BT_Register_UpdateStatusGA(BTRegisterID, GAStatus.rejected.ToString(), strUser, rejectReason)
    '        LoadDataGrid()
    '        '
        ...
        Try
            ...
            If dtRegister.Rows.Count > 0 Then
                ...
                Dim params As String = String.Format( _
                    "btid={0}&bttype={1}&loc={2}&fdate={3}&tdate={4}&div={5}&dep={6}&sec={7}&group={8}&ecode={9}&ename={10}&rpage={11}&rpsize={12}&spage={13}&spsize={14}&jpage={15}&jpsize={16}&qpage={17}&qpsize={18}", _
                    ...
                    grvBTSubmitted.PageIndex, grvBTSubmitted.SettingsPager.PageSize, grvBTRejected.PageIndex, grvBTRejected.SettingsPager.PageSize, grvBTNoRequest.PageIndex, grvBTNoRequest.SettingsPager.PageSize)
                postBackUrl.Append(String.Format("?id={0}&back=GAAdvanceMgmt.aspx&params={1}", btID, params.Replace("&", ";amp;").Replace("=", ";eq;")))
    ...
    Protected Sub grvBTRegister_HtmlRowPrepared(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridView.ASPxGridViewTableRowEventArgs) _
        Handles grvBTRegister.HtmlRowPrepared, grvBTRejected.HtmlRowPrepared, grvBTNoRequest.HtmlRowPrepared
